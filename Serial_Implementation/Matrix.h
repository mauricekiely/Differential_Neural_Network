...
public:
    ...
    // Copy Constructor for other type
    template <class U>
    Matrix(const Matrix<U>& rhs) : myRows(rhs.num_rows()), myCols(rhs.num_cols()){
        myVector.resize(rhs.num_rows() * rhs.num_cols());
        copy(rhs.begin(), rhs.end(), myVector.begin());
    }

    // Copy Assignment for other type
    template <class U>
    ...
    Matrix dot(const Matrix& rhs) const {
        if (myCols != rhs.num_rows()) {
            throw invalid_argument("Matrix dimensions must match for dot product.");
        }
